...
* cd "C:\Users\t.scarelli\Dropbox\7_Research\2_Master_Revisited_in_Stata\data" /* PSE */
* cd "C:\Users\thiag\Dropbox\7_Research\2_Master_Revisited_in_Stata\data" /* Laptop */

* Alternatively, saving a path into a global
* global datafolder C:\Users\t.scarelli\Dropbox\7_Research\2_Master_Revisited_in_Stata\data
* use "${datafolder}input.dta"

* Open log
log using 1_define_identifiers, replace text
...
* In case of potential conflicts with variable names, use the pseudofunction scalar() 
display scalar(c)

* Return stored scalars
scalar dir
...
* Note the difference for column and row vectors
matrix m1 = 1 \ 2 \ 3 \ 4
matrix list m1
...
* Macros
local local_macro = 59
display `local_macro'

global global_macro = 85
display $global_macro

* Return stored macros
macro dir

* Return stored elements
return list

* Return elements from estimations
ereturn list
...
* Access help
help something /* help for a particular commands */
search something /* search keyword */
hsearch something /* search keyword in all help files */
findit something /* extend search into external sources, packages */
...
* Draw three random variable from a multivariate distribution
drawnorm x1 x2 x3, n(100) means(m) sds (sd) corr(C)
...
* Stata estimates table command produces a summary of the coefficients
estimates dir
estimates table model_1 model_2, b(%9.2f) 

* Estout package provides extra options for output tables
esttab model_1 model_2, se star(* 0.10 ** 0.05 *** 0.01) label

* Save the results to a text file
esttab model_1 model_2 using "reg_table.txt", plain b(%9.2f) plain not line replace

* Export a latex table
estout model_1 model_2 using "reg_table.tex", style(tex) replace 
